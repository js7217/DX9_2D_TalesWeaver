...
HRESULT CHorseBee::LateInit()
{
	m_tData.strName = L"말벌";
...
void CHorseBee::MoveFrame(float fSpeed)
{
	switch (m_eCurState)
	{
	case CMonster::STAND:
	case CMonster::WALK:
		m_tFrame.fCurFrame += m_tFrame.fMaxFrame * m_pTimeMgr->GetDeltaTime() * 8.f;
...
void CHorseBee::MakeWorldMatrix()
{
...
HRESULT CHorseBee::Create(float fX, float fY, UNIT_DATA tData)
{
	CHorseBee* pInstance = new CHorseBee;
